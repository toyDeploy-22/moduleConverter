...
// Local:
import { pdfUpload, pdfCheck, Pdf2JS } from "./Functions/pdf-2-js.js";
...
const pdf_Js_conv = Express.Router();
let downloadFile = {};
const conversionFolder = join(dirname(fileURLToPath(import.meta.url)), "Conversion");

const typeRequired = "JS";

pdf_Js_conv.options("/pdf-2-js", cors())
pdf_Js_conv.post("/pdf-2-js", cors(), pdfUpload, async(req, res, next )=>{ 
  ...
  try {
    if(req.file.originalname) {
        ...
        switch (checker.code.toString()[0]) {
          ...
          const newJsFile = await Pdf2JS(pdfLoad.data);
// can use "res.download(newFile.file.toString());" but nothing can be done after it, contrary to res.writeHead.
      ...
      pdf_Js_conv.get("/pdf-2-js/getFile", async(req, res, next )=>{ 
        try {
