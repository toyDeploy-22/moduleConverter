...
const jsonMaking = (newFileName) => {
    ...
    pdfUtil.pdfToText(pdfPath, function(err, data) {
        ...
        newJsonDoc.on("error", (err)=>{
            result.error = true;
            result.msg = err;
            console.error("An error occured during writing: " + err);
            return result;
        });
        newJsonDoc.end();
        }, 950)}    
...
async function Pdf2Json() { 
    ...
    jsonMaking(newJsonFile);
    // await writeFile(join(join(conversionFolder, "./JSON"), newJsonName), jsonBuffer); 
    
    result.error = false;
    result.code = 201;
    result.newFileName = newJsonFile;
    result.uploadFolder = join(conversionFolder,"./UPLOAD");
    result.originalFilePath = join(join(conversionFolder,"./UPLOAD"), newPdfFile);
    result.filePath = join(join(conversionFolder, "./JSON"), newJsonFile);
    result.msg = "PDF file successfully converted to JSON. Ready for download.";
    return result;
