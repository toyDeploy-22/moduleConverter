...
// Local:
import { txtUpload, txtCheck, txt2Json } from './Functions/txt-2-json.js';
// 3rd Party:
import cors from "cors";
import Express from "express";

// destruct/constants/variables
const txt_json_conv = Express.Router();
let downloadFile = {};

txt_json_conv.post("/txt-2-json", cors(), txtUpload, async(req, res, next )=>{ 
  
  try {
    if(req.file.originalname) {
        console.log(req.body, req.file); 
        const checker = txtCheck(req.file); 
        console.log("TXT checker :" + checker);
        ...
        switch (checker.code.toString()[0]) {
          ...
          break; 
          case "2":   
          const newJsonFile = await txt2Json();
// can use "res.download(newFile.file.toString());" but nothing can be done after it, contrary to res.writeHead.
      ...
      txt_json_conv.get("/txt-2-json/getFile", async(req, res, next )=>{ 
        try {
